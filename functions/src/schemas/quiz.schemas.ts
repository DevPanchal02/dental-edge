...
export const QuizAttemptSchema = z.object({
    ...
    status: z.enum(['initializing', 'active', 'reviewing_summary', 'completed', 'in-progress']), // Added 'in-progress' for backend
    score: z.number().optional(),
    
    createdAt: z.number().optional(), // Firestore timestamp usually, but client sends number
...
// --- Identifiers Schema (Used for API requests) ---
export const QuizIdentifiersSchema = z.object({
    topicId: z.string(),
    sectionType: z.string(), // We keep this loose string input, define refinement if needed
    quizId: z.string(),
});
...
export type QuizIdentifiers = z.infer<typeof QuizIdentifiersSchema>;